        ...
        <main className="relative min-h-screen w-full overflow-hidden font-geist">
            ...
            <div className="relative z-10 flex min-h-screen items-center justify-center p-4">
                <div className="flex w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-in fade-in zoom-in-95 duration-500">
                    ...
                    {/* Right Column: Form */}
                    <div className="flex w-full flex-col justify-center bg-white p-8 lg:w-7/12 lg:p-16 relative">
                        {step === 1 && (
                            <Link to="/login" className="absolute top-8 left-8 text-zinc-400 hover:text-zinc-700 transition-colors flex items-center gap-1 text-sm font-medium">
                                <ArrowLeft size={16} /> Kembali
                            </Link>
                        )}
                        {step === 2 && (
                            <button onClick={() => {setStep(1); setAlertMsg(null);}} className="absolute top-8 left-8 text-zinc-400 hover:text-zinc-700 transition-colors flex items-center gap-1 text-sm font-medium">
                                <ArrowLeft size={16} /> Batal
                            </button>
                        )}

                        <div className="mb-8 mt-4">
                            <h2 className="text-3xl font-bold text-zinc-900">{step === 1 ? "Forgot Password" : "Create New Password"}</h2>
                        ...
                        {step === 1 ? (
                            <form onSubmit={handleCheckEmail} className="flex flex-col gap-6">
                                <TextField
                                    label="Email Address"
